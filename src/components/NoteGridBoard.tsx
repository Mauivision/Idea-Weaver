    ...
    >
          ...
          >
            ...
            >
              ...
              {isEditing ? (
                ...
                >
                  {note.content || <span style={{ opacity: 0.35, fontStyle: 'italic' }}>Click to write...</span>}
                </Typography>
              )}
      ...
      {linkMenuAnchor && (
        ...
        >
          {ideas
            .filter((i) => {
              const source = ideas.find((x) => x.id === linkMenuAnchor.ideaId);
              return i.id !== linkMenuAnchor.ideaId && !source?.connections.includes(i.id);
            })
            .map((target) => (
              <MenuItem
                key={target.id}
